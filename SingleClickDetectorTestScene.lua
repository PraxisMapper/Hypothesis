...
local touchDetector = {} -- replacing CellTapSensors here for this experiment on improving performance
...
local function DetectLocationClick(event)
    ...
    -- zoomed out grid: 160x200 * 5 = 800 x 1000 pixels
    -- each pixel is a Cell 11?
    -- zoomed in grid: 320 x 400 * 3 = 960 x 1200 pixels
    -- each pixel here is 1/4 cell 11?
    ...
    local pixelshiftY = screenY - centerY
    local plusCodeShiftX = 0
    local plusCodeShiftY =0

    --TODO: check on these numbers, might be backwards or might need to be positive
    if (bigGrid) then
        pixelshiftX -= 4
        pixelshiftY -= 5
    ...
    --we always display the curent Cell8 in the center of the screen, so lets use that as our basis here
...
function scene:create(event)
    ...
    touchDetector = display.newRect(sceneGroup, 0, 0, 720, 1280)
    touchDetector:addEventListener("touch", DetectLocationClick)
